...
class BookModel(models.Model):
    isbn = models.CharField(_('ISBN'), validators=[isbn_validator], max_length=13, primary_key=True)
    ...
    book_image = models.ImageField(_('Book Image'), blank=True, null=True)
    ...
    slug = AutoSlugField(populate_from=['book_title'], unique=True, allow_unicode=True,
                         slugify_function=my_slugify_function)

    # things that may be inherited from another app
    book_comment = models.TextField(_('Comment'), blank=True, null=True)
    book_fav = models.BooleanField(_('Favorite'), default=False)
    book_like = models.BooleanField(_('Like'), default=False)
    book_dislike = models.BooleanField(_('Dislike'), default=False)
    book_unread = models.BooleanField(_('Unread Books'), default=True)
